...
Splicer.prototype.push = function (...args) {
    var args = [this._streams.length, 0].concat([].slice.call(args));
...
Splicer.prototype.unshift = function (...args) {
    this.splice.apply(this, [0, 0].concat([].slice.call(args)));
    return this._streams.length;
};
Splicer.prototype.splice = function (...args) {
    var self = this;
    var ix;
    var start = args[0];
    var removeLen = args[1];
    if (typeof start === "string") {
        ix = this.indexOf(start);
    }
    else
        ix = start;
    var args = [ix].concat([].slice.call(args, 1));
