...
import { TextureFormat, BufferUsage, FeatureName, StoreOp, TextureUsage, CanvasCompositingAlphaMode, LoadOp, TextureViewDimension, TextureAspect, IndexFormat, TextureDimension } from './WebGPU/webgpuConstants.js';
...
import { WebGPUBundleList, WebGPURenderItemScissor, WebGPURenderItemStencilRef, WebGPURenderItemViewport, WebGPURenderItemBlendColor } from './WebGPU/webgpuBundleList.js';
...
class WebGPUEngine extends Engine {
    ...
    _initializeMainAttachments() {
        ...
        if (this._options.antialiasing) {
            ...
            mainColorAttachments = [{
                    ...
                    storeOp: StoreOp.Store // don't use StoreOp.Clear, else using several cameras with different viewports or using scissors will fail because we call beginRenderPass / endPass several times for the same color attachment!
                }];
        }
        else {
            mainColorAttachments = [{
    ...
    createTexture(url, noMipmap, invertY, scene, samplingMode = Constants.TEXTURE_TRILINEAR_SAMPLINGMODE, onLoad = null, onError = null, buffer = null, fallback = null, format = null, forcedExtension = null, mimeType, loaderOptions, creationFlags, useSRGBBuffer) {
        return this._createTextureBase(url, noMipmap, invertY, scene, samplingMode, onLoad, onError, (texture, extension, scene, img, invertY, noMipmap, isCompressed, processFunction, samplingMode) => {
            ...
            if (!((_a = texture._hardwareTexture) === null || _a === void 0 ? void 0 : _a.underlyingResource)) { // the texture could have been created before reaching this point so don't recreate it if already existing
